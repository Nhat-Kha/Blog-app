...
export default function CommentSection({ postId, post }) {
  ...
  useEffect(() => {
    const user = async () => {
      try {
        const res = await fetch(apiList.getAllUser);
        if (res.ok) {
          const data = await res.json();
          setMainUser(data.allUser);
        }
      } catch (error) {
  ...
  console.log("comments:", newMainUser);
  return (
      ...
      {newMainUser.length > 0 ? (
        <div className="flex items-center gap-1 my-5 text-gray-500 text-sm">
          ...
          <img
            ...
            src={newMainUser[0].profilePicture}
            alt=""
          />
          <Link
